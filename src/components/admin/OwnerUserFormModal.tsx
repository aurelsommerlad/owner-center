...
/** Create or edit an AdminOwnerUser - same form, only the action + prefilled values differ. */
export function OwnerUserFormModal({
  ...
  async function handleSubmit(formData: FormData) {
    ...
    setError(null);
    const result = isEdit
      ? await updateOwnerUserAction(user!.id, ownerId, formData)
      : await createOwnerUserAction(formData);
    setPending(false);
    if (!result.ok) {
  ...
  return (
    <>
      ...
      <AdminModal
        ...
        title={isEdit ? "Nutzer bearbeiten" : "Nutzer hinzufügen"}
      >
